...
!
 ...
 real(kind=PR) :: sum_Lx, sum_Ly, sum_Lz, L_d		!!! to x-y plane	
 integer :: rotate_ref = 0				!!! Define which feature rotation based on (0 for central sink, 1 for disc) 
!
...
!
 ...
 real(kind=DP), parameter :: z_min = -30.
 real(kind=DP), parameter :: z_max = 30. 
!
 real(kind=PR), allocatable :: pv_x(:)			!!! Define arrays for PV diagram  
 real(kind=PR), allocatable :: pv_z(:)			!!! outputs
 real(kind=PR), allocatable :: pv_vy(:)
...
!
 integer :: pv_print = 0				!!! Choose whether (1) to generate P-V data or not (0)
!
 real(kind=PR) :: inclin				!!! Define inclination and Keplerian restriction parameters, then read
 real(kind=PR) :: restrkep				!!! in from generate.sh, converting percentage -> fraction for v_K variable
! 
...
!
!
!!! Align disc to x-y plane based on primary sink angular momentum
!
!
!
...
!
 if (rotate_ref .eq. 0) then				!!! Rotate disc with reference
	theta_r = acos(abs(sink(nsin)%angmom(3))/ &	!!! to central sink L vector
...
!
	do p=1,ptot
		if ( (sqrt(parray(1,p)**2+parray(2,p)**2)*rscale*206265. .lt. 10.) .and. &
		   (sqrt(parray(3,p)**2)*rscale*206265. .lt. 1.) ) then
!
			sum_Lx = sum_Lx + parray(MASS, p)*mscale*2.e30* &
			(v(3,p)*vscale*1000.*parray(2,p)*rscale*3.e+16 - &
			v(2,p)*vscale*1000.*parray(3,p)*rscale*3.e+16)
!
			sum_Ly = sum_Ly + parray(MASS, p)*mscale*2.e30* &
			(v(1,p)*vscale*1000.*parray(3,p)*rscale*3.e+16 - &
			v(3,p)*vscale*1000.*parray(1,p)*rscale*3.e+16)
!
			sum_Lz = sum_Lz + parray(MASS, p)*mscale*2.e30* &
			(v(2,p)*vscale*1000.*parray(1,p)*rscale*3.e+16 - &
			v(1,p)*vscale*1000.*parray(2,p)*rscale*3.e+16)
		endif
  	enddo
!
	  L_d = sqrt(sum_Lx**2 + sum_Ly**2 + sum_Lz**2)
!
	  theta_r = acos(abs(sum_Lz) / L_d)
 endif	
!
 theta_d = theta_r * (180. / 3.14159)			!!! Convert radians output for theta to degrees
 theta_incl = (theta_d + inclin) / (180. / 3.14159)	!!! Offset system with inclination desired and reset to radians
 costheta = cos ( theta_incl )				!!! Reset absolute theta value to cos(theta_r + inclin)
...
!
	...
	allocate(pv_z(1:trho_count))								    !!! restrictions met, output data in array
	allocate(pv_vy(1:trho_count))								    !!! for PV diagram (raw, histogram and fit)
...
!
	...
	do p=1,ptot
		if( (rho(p)*rhoscale .gt. rhorestr) .or. (temp(p) .lt. trestr) ) then
			...
			pv_z(i) = parray(3,p)*rscale*206265.
			pv_vy(i) = v(2,p)*vscale  
			write(2, '(3E15.7,2X)') pv_x(i), pv_z(i), pv_vy(i)
			i = i + 1
		endif
	enddo
!
...
!
			...
			do i=1, trho_count
				if ( ( pv_x(i) .gt. r_arr1(rint) ) .and. ( pv_x(i) .lt. r_arr2(rint) ) &
				   .and. (pv_vy(i) .gt. v_arr1(vint) ) .and. (pv_vy(i) .lt. v_arr2(vint) ) & 
				   .and. (pv_z(i) .gt. z_min ) .and. (pv_z(i) .lt. z_max ) ) then
