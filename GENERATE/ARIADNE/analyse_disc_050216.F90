...
!
	do p=1,ptot
		if ( (sqrt((parray(1,p)-sink(nsin)%r(1))**2+(parray(2,p)-sink(nsin)%r(2))**2)*rscale*206265. .lt. 100.) ) then
!
...
!
 ...
 write(*,*) "Rotation angle (theta): ", theta_d
...
!
 do p=1,ptot						!!! Rotate about x-axis
	ytmp=parray(2,p)   
	ztmp=parray(3,p)
	vytmp=v(2,p)  
	vztmp=v(3,p)
...
!
	do p=1,ptot
		xtmp=parray(1,p)			!!! Rotate about z-axis
		ytmp=parray(2,p)   
		vytmp=v(2,p)  
		vxtmp=v(1,p)
