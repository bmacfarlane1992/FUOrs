...
!
 if (rotate_ref .eq. 0) then				!!! Rotate disc with reference
	costheta = abs(sink(nsin)%angmom(3))/ &		!!! to central sink L vector
...
!
 sintheta=sqrt(1-costheta**2)				!!! Rotate to either sink or disc L z component
...
!
!!! Determine angle to rotate such that disc is on
!!! disc midplane is centred on z = 0 (for 0 deg. inclination case)
!
 do p=1,ptot
	ztmp=parray(3,p)			!!! Rotate about y-axis
	xtmp=parray(1,p)   
	vztmp=v(3,p)  
	vxtmp=v(1,p)
	parray(3,p)=costheta*ztmp-sintheta*xtmp
	parray(1,p)=sintheta*ztmp+costheta*xtmp
	v(3,p)=costheta*vztmp-sintheta*vxtmp
	v(1,p)=sintheta*vztmp+costheta*vxtmp 
 enddo
! 
 do s=1,stot
	xtmp=sink(s)%r(1)  
	...
	vztmp=sink(s)%v(3)
	vxtmp=sink(s)%v(1)
	sink(s)%r(3)=costheta*ztmp-sintheta*xtmp
	sink(s)%r(1)=sintheta*ztmp+costheta*xtmp 
	sink(s)%v(3)=costheta*vztmp-sintheta*vxtmp
	sink(s)%v(1)=sintheta*vztmp+costheta*vxtmp     
 end do
!
!!! Now incline disc with user defined angle, only rotating about x and z axes
!
 costheta = cos ( inclin * (pi / 180.) )
